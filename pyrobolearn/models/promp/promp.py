...
from pyrobolearn.models.gaussian import Gaussian, compute_gaussian_product, compute_joint_gaussian
from pyrobolearn.models.promp.canonical_systems import LinearCS, CS
...
class ProMP:  # Model
    ...
    * blending / sequencing
    ...
    def __init__(self, num_dofs: int, weight_size: Optional[int] = None,
                 output_dim: Optional[int] = None,
                 weights: Optional[Union[npt.ArrayLike, Gaussian]] = None,
                 weights_covariance: Optional[npt.ArrayLike] = None, canonical_system: Optional[CS] = None,
                 noise_covariance: Union[float, npt.ArrayLike]=1., Phi: Optional[BasisMatrix] = None,
                 promps: Optional[List["ProMP"]] = None) -> None:
        """
        ...
        Args:
            ...
            output_dim (int, None): output dimensionality. if None, it will be set to `2*num_dofs` (=2D).
            weights (None, np.float[DM], Gaussian): mean of the weigths, or Gaussian weight distribution.
            weights_covariance (None, np.float[DM,DM]): covariance on the weights.
            canonical_system (None, CS): canonical system.
            noise_covariance (float, np.float[2D], np.float[2D,2D]): covariance on the noise.
        ...
        if Phi is not None and not isinstance(Phi, BasisMatrix):
            raise TypeError("Expecting the given basis matrix Phi to be an instance of "
                            f"`BasisMatrix`, instead got: {Phi}.")
        ...
        if promps:
            ...
            for i, promp in enumerate(promps):
                if not isinstance(promp, ProMP):
                    raise TypeError(f"The {i}th item is not an instance of ProMP.")
            ...
            for i in range(0, len(promps) - 1):
                ...
                if promp.Phi is None:
                    raise ValueError(f"The {i}th ProMP doesn't have a basis matrix defined.")

                # check if each ProMP has the same dimensionality (i.e. the same number of DoF)
                if promp.dim != next_promp.dim:
                    raise ValueError(f"The {i}th and {i+1}th ProMPs have different dimensions.")

                # check if each ProMP has the same number of time steps
                if promp.cs.num_timesteps != next_promp.cs.num_timesteps:
                    raise ValueError(f"The {i}th and {i+1}th ProMPs have different number of time steps.")
        ...
        # else, define one promp
            ...
            if Phi is not None:
                # basis matrix (shape should be: DMxd where D=nb of DoFs (and D<=d<=2D), and M=nb of basis fcts)
                output_dim = Phi.shape[1]
                # num_dofs = Phi.shape[1]  # / 2
            else:
                if output_dim is None:
                    output_dim = 2 * num_dofs
            ...
            if isinstance(weights, Gaussian):
                ...
                if Phi is not None:
                    if Phi.shape[0] != weights.mean.size:
                        raise ValueError("Mismatch between the dimensions of Phi and the weights; got "
                                         f"Phi.shape[0]={Phi.shape[0]} and weights.size={weights.mean.size}.")
            else:
                ...
                if Phi is not None:
                    ...
                    if weights:
                        if weights.size != size:
                            raise ValueError(
                                f"Mismatch between the dimensions of Phi and the weights; got Phi.shape[0]={size} "
                                f"and weights.size={weights.size}.")
                elif weights:
                    size = weights.size
                    ...
                    if weight_size is None:
                        raise ValueError("Expecting the weight_size to be given.")
                    size = weight_size
            ...
            if isinstance(noise_covariance, (float, int)):
                noise_covariance = noise_covariance * np.identity(output_dim)
            elif isinstance(noise_covariance, np.ndarray):
                if len(noise_covariance.shape) == 1:
                    noise_covariance = np.diag(noise_covariance)
                if noise_covariance.shape != (output_dim, output_dim):
                    raise ValueError(f"Expecting the given noise covariance to be of shape ({output_dim},{output_dim})"
                                     f", but instead got shape: {noise_covariance.shape}.")
            else:
                raise TypeError("Expecting the given noise covariance to be an int, float, or np.ndarray[2D,2D], but "
                                f"instead got: {type(noise_covariance)}.")
            self._noise = Gaussian(np.zeros(output_dim), noise_covariance)
    ...
    @property
    def canonical_system(self) -> CS:
        """Return the canonical system."""
        return self.cs

    @property
    def num_mps(self) -> int:
    ...
    @property
    def priority_fct(self) -> Callable:
        """Return the priority exponent function which accepts the phase(s) as an input."""
        return self._alpha_fct

    @priority.setter
    def priority(self, priority: Union[None, float, int, Callable]) -> None:
        ...
        if priority is None:
            ...
            self._alpha_fct = lambda _: priority
        ...
        #     self._alpha = priority
            ...
            try:
                ...
                for _ in range(self.cs.num_timesteps):  # for one period
                    phase = self.cs.step()
                    priority_value = priority(phase)
                    if not isinstance(priority_value, (float, int)):
                    ...
                    if priority_value < 0 or priority_value > 1:
                        raise ValueError("The callable function returned a priority which is not between 0 and 1; "
                                         f"given the phase = {phase}, it returned the priority={priority_value}.")

                # check if the callable priority accepts the phase values all at once
                self.cs.reset()
                phase = self.cs.rollout()
                priority_value = priority(phase)
                if not isinstance(priority_value, np.ndarray):
                    raise TypeError("The callable function/class doesn't return an np.array given the phase values.")
                if np.all(priority_value < 0) or np.all(priority_value > 1):
    ...
    @staticmethod
    def copy(other: "ProMP", deep: bool = True) -> "ProMP":
    ...
    def basis(self, phase: Union[float, npt.ArrayLike]) -> np.ndarray:
        r"""
        Return the basis matrix evaluated at the given phase(s).

        Args:
            phase (np.float[T], float): phase value(s).

        Returns:
            np.float[DM,2D], np.float[DM,T,2D]: basis matrix evaluated at the given phase(s).
        """
        return self.Phi(phase)

    def weighted_basis(self, phase: Union[float, npt.ArrayLike]) -> np.ndarray:
        r"""
        Return the weighted basis evaluated at the given phase(s), that is, :math:`Phi(s) * w`, where :math:`w` is
        the weight mean vector, and the product is a broadcast product (and not a dot/scalar product).

        Args:
            s (np.float[T], float): phase value(s).

        Returns:
            np.float[DM,T,2D]: weighted basis matrix (this has the same shape as the basis matrix Phi(s)).
        """
        return (self.Phi(phase).T * self.weights.mean).T

    def predict_conditional(self, phase: Union[float, npt.ArrayLike],
                            weights: Optional[Union[Gaussian, npt.ArrayLike]] = None) -> Union[Gaussian, List[Gaussian]]:
        r"""
        ...
        Args:
            ...
            weights (Gaussian, np.float[DM], None): weight distribution(s) or weight means (array). If None, it will
              take the mean/mode of the default weight distribution.

        Returns:
            Gaussian, list[Gaussian]: conditional output distribution for each phase value
        """
        # if multiple promps that need to be combined
        if self.promps:
            gaussians = [promp.predict_conditional(phase) for promp in self.promps]
            priorities = [promp.priority_fct(phase) for promp in self.promps]

            # if one phase value
            if isinstance(gaussians[0], Gaussian):
                return compute_gaussian_product(gaussians, priorities)

            # if multiple phases
            return [compute_gaussian_product(gaussian, priority) for gaussian, priority in zip(gaussians, priorities)]

        # if one promp
        if weights is None:
            weights = self.weights.mean
        elif isinstance(weights, Gaussian):
            weights = weights.mean
        if isinstance(phase, (float, int)):
            return self.Phi(phase).T.dot(weights) + self.noise
        return [(self.Phi(phase_).T.dot(weights) + self.noise) for phase_ in phase]

    def predict_marginal(self, phase: Union[float, npt.ArrayLike],
                         weights: Optional[Gaussian]=None) -> Union[Gaussian, List[Gaussian]]:
        r"""
        ...
        Args:
            phase (float, np.float[T]): phase value(s).
            weights (None, Gaussian): weight distribution. If None, it will use the default weight distribution.

        Returns:
            Gaussian, list[Gaussian]: marginal output distribution for each phase value
        """
        # if multiple promps that need to be combined
        if self.promps:
            gaussians = [promp.predict_marginal(phase) for promp in self.promps]
            priorities = [promp.priority_fct(phase) for promp in self.promps]
        ...
        elif not isinstance(weights, Gaussian):
            raise TypeError(f"Expecting the given weights to be a Gaussian distribution, but got {type(weights)}")
        if isinstance(phase, (float, int)):
            return self.Phi(phase).T * weights + self.noise
        return [(self.Phi(phase_).T * weights + self.noise) for phase_ in phase]

    def predict(self, phase: Union[float, npt.ArrayLike], sample: bool = False,
                weights: Optional[Gaussian] = None, method: str = 'conditional') -> np.ndarray:
        r"""
        ...
        Returns:
            np.float[2D], np.float[T,2D]: output mean(s).
        """
        # Prediction
        if method == 'conditional':
            gaussians = self.predict_conditional(phase, weights)
        elif method == 'marginal':
            gaussians = self.predict_marginal(phase, weights)
        else:
    ...
    def predict_proba(self, phase: Union[float, npt.ArrayLike], method: str = 'marginal', return_gaussian: bool = True
                      ) -> Union[Gaussian, List[Gaussian], Tuple[np.ndarray, np.ndarray]]:
        r"""
        ...
        Returns:
            if return_gaussian:
                Gaussian, list[Gaussian]: gaussian(s) for each input phase value.
            else:
                np.float[2D], np.float[T,2D]: output mean(s).
                np.float[2D,2D], np.float[T,2D,2D]: output covariance(s).
        """
        # Predict
        if method == 'conditional':
            y = self.predict_conditional(phase)
        elif method == 'marginal':
            y = self.predict_marginal(phase)
        else:
            raise NotImplementedError("The given 'method' argument has not been implemented. Please choose "
                                      "between 'conditional' and 'marginal'.")
    ...
    def step(self, tau: float = 1., sample: bool = False, method: str = 'marginal') -> np.ndarray:
        r"""
        ...
        Returns:
            np.float[2D]: predicted output.
        """
        phase = self.cs.step(tau=tau)
        return self.predict(phase, sample=sample, method=method)

    def step_proba(self, tau: float = 1., method: str = 'marginal',
                   return_gaussian: bool = True) -> Union[Gaussian, Tuple[np.ndarray, np.ndarray]]:
        r"""
        ...
        Returns:
            if return_gaussian:
                ...
                np.float[2D]: output mean.
                np.float[2D,2D]: output covariance.
        """
        phase = self.cs.step(tau=tau)
        return self.predict_proba(phase, method=method, return_gaussian=return_gaussian)

    def rollout(self, tau: float = 1., sample: bool = False, method: str = 'marginal') -> npt.ArrayLike:
        r"""
        ...
        Returns:
            np.float[T,2D]: predicted outputs.
        """
        # reset system
        self.reset()

        # rollout with the canonical system
        phases = self.cs.rollout(tau=tau)

        # return predictions
        return self.predict(phases, sample=sample, method=method)

    def rollout_proba(self, tau: float = 1., method: str = 'marginal',
                      return_gaussian: bool = True) -> Union[List[Gaussian], Tuple[np.ndarray, np.ndarray]]:
        r"""
        ...
        Returns:
            if return_gaussian:
                ...
                np.float[T,2D]: output means.
                np.float[T,2D,2D]: output covariances.
        """
        # reset system
        self.reset()
        ...
        # return predictions
        return self.predict_proba(phases, method=method, return_gaussian=return_gaussian)

    def sample_weights(self, size: Optional[int] = None, seed: Optional[int] = None) -> np.ndarray:
        """
        ...
        Returns:
            np.float[DM], np.float[N,DM]: samples.
        """
        return self.weights.sample(size=size, seed=seed)

    def sample_trajectory(self, size: Optional[int] = None) -> np.ndarray:
        r"""
        ...
        Returns:
            np.float[T,2D], np.float[N,T,2D]: trajectory/trajectories.
        """
    ...
    def sample_from_prediction(self, phase: float, size: Optional[int] = None, seed: Optional[int] = None,
                               method: str = 'conditional') -> np.ndarray:
        r"""
        ...
        Returns:
            np.float[2D], np.float[N,2D]: sample(s).
        """
        if not isinstance(phase, (float, int)):
            raise TypeError("Expecting only one phase value (float).")
        gaussian = self.predict_proba(phase, method=method, return_gaussian=True)
        return gaussian.sample(size=size, seed=seed)

    def sample_from_conditional(self, phase: float, size: Optional[int] = None,
        ...
        if not isinstance(phase, (float, int)):
            raise TypeError("Expecting only one phase value (float)")
        gaussian = self.predict_conditional(phase)
        return gaussian.sample(size=size, seed=seed)

    def sample_from_marginal_likelihood(self, phase: float, size: Optional[int] = None,
                                        seed: Optional[int] = None) -> np.ndarray:
        r"""
        ...
        if not isinstance(phase, (float, int)):
            raise TypeError("Expecting only one phase value (float).")
        gaussian = self.predict_marginal(phase)
        return gaussian.sample(size=size, seed=seed)

    def likelihood(self, y: npt.ArrayLike, phase: Optional[Union[float, npt.ArrayLike]] = None) -> float:
        r"""
        ...
        Args:
            y (np.float[2D], np.float[T,2D]): output vector(s) to evaluate the likelihood. It can be an output
              vector at one particular time step, or an output vector for each time step.
            phase (float, np.float[T], None): the corresponding time step(s). If None, it will generate the same number
        ...
        period = y.shape[0]  # period = T
        if phase is None:
            phase = np.linspace(0.,1.,period)
        if isinstance(phase, (float, int)):
            phase = phase * np.ones(period)
        if len(phase) != period:
            raise ValueError("Expecting the number of phase values to be the same as the number of output vectors")
        gaussians = self.predict_conditional(phase)
        return np.prod([gaussian.pdf(yt) for yt, gaussian in zip(y, gaussians)])

    # alias
    pdf = likelihood

    def log_likelihood(self, y: npt.ArrayLike, phase: Optional[Union[float, npt.ArrayLike]] = None) -> float:
        r"""
        ...
        """
        return np.log(self.likelihood(y, phase))

    # alias
    log_pdf = log_likelihood

    def marginal_likelihood(self, y: npt.ArrayLike, phase: Optional[Union[float, npt.ArrayLike]] = None) -> float:
        r"""
        ...
        if len(phase) != period:
            raise ValueError("Expecting the number of phase values to be the same as the number of output vectors")
        gaussians = self.predict_marginal(phase)
        return np.prod([gaussian.pdf(yt) for yt, gaussian in zip(y, gaussians)])

    def log_marginal_likelihood(self, y: npt.ArrayLike, phase: Optional[Union[float, npt.ArrayLike]] = None) -> float:
        r"""
        ...
        """
        # return np.log(self.marginal_likelihood(y, phase))
        if len(y.shape) == 1:
            if not isinstance(phase, (float, int)):
                raise TypeError("One sample is given but not its associated phase value (float number)")
            gaussian = self.predict_marginal(phase)
        ...
        if len(phase) != period:
            raise ValueError("Expecting the number of phase values to be the same as the number of output vectors")
        gaussians = self.predict_marginal(phase)
        return np.sum([gaussian.log_pdf(yt) for yt, gaussian in zip(y, gaussians)])

    def joint_distribution(self, y: npt.ArrayLike, Phi_or_phase: Union[float, npt.ArrayLike],
                           y_cov: Optional[npt.ArrayLike]=None) -> Gaussian:
        r"""
        ...
        Args:
            y (np.float[2D]): output vector.
            Phi_or_phase (np.float[DM,2D], float): basis matrix evaluated at a particular phase, or phase value.
              If the phase is given, the basis matrix will computed internally.
            y_cov (np.float[2D,2D], None): desired covariance.
        ...
        if isinstance(Phi_or_phase, (float, int)): # phase value
            Phi = self.Phi(Phi_or_phase)
        else:
            Phi = Phi_or_phase

        # compute joint distribution between weights and output vector
        y_dist = Phi.T * self.weights + Gaussian(np.zeros(y.shape[-1]), y_cov)
        cov = self.weights.cov.dot(Phi)
        return compute_joint_gaussian(self.weights, y_dist, cov)

    def posterior_weights(self, y: npt.ArrayLike, Phi_or_phase: Union[float, npt.ArrayLike],
                          y_cov: Optional[npt.ArrayLike]=None) -> Gaussian:
        r"""
        ...
        joint = self.joint_distribution(y, Phi_or_phase, y_cov)
    ...
    def compute_loss(self, Y: Union[npt.ArrayLike, List[npt.ArrayLike]], kind: str = 'marginal_log_likelihood') -> float:
        r"""
        Compute the loss on the whole given data.

        Args:
            Y (np.float[N,T,2D], list[np.float[T,2D]]): state trajectories.
        ...
        if kind == 'marginal_log_likelihood':
            return np.sum([self.log_marginal_likelihood(y, phase) for y, phase in zip(Y, phases)])
        if kind == 'log_likelihood':
            return np.sum([self.log_likelihood(y, phase) for y, phase in zip(Y, phases)])
        if kind == 'likelihood':
            return np.sum([self.likelihood(y, phase) for y, phase in zip(Y, phases)])
        if kind == 'marginal_likelihood':
            return np.sum([self.marginal_likelihood(y, phase) for y, phase in zip(Y, phases)])
        raise ValueError("The specified kind of loss has not been implemented")

    def expectation_maximization(self, Y: Union[npt.ArrayLike, List[npt.ArrayLike]], num_iters: int = 1000,
                                 threshold: float = 1e-4, verbose: bool = False) -> Dict[str, Any]:
        r"""
        ...
        for it in range(num_iters):
            ...
            for y in Y:
                # compute phase
                phases = np.linspace(0., 1., len(y))

                # compute weight vector
                y = y.reshape(-1)  # shape: 2DT
                Phi = np.hstack([self.Phi(phase) for phase in phases])  # DMx2DT
            ...
            if np.abs(loss - prev_loss) <= threshold:
                if verbose:
                    print(f"Convergence achieved at iteration {it+1} with associated loss: {loss}.")
    ...
    def maximum_marginal_likelihood(self, Y: Union[npt.ArrayLike, List[npt.ArrayLike]], prior_reg: float = 1.,
                                    num_iters: int = 1000, threshold: float = 1e-4,
    ...
    def linear_ridge_regression(self, Y: Union[npt.ArrayLike, List[npt.ArrayLike]],
                                prior_reg: float = 1.) -> Dict[str, Any]:
        r"""
        ...
        for y in Y:
            ...
            y = y.reshape(-1)  # shape: 2DT
            Phi = np.hstack([self.Phi(phase) for phase in phases])  # shape: DMx2DT
    ...
    def imitate(self, Y: Union[npt.ArrayLike, List[npt.ArrayLike]], prior_reg: float = 1.,
                method: str = 'lrr', num_iters: int = 1000, threshold: float = 1e-4,
                verbose: bool = False) -> Dict[str, Any]:
        r"""
    ...
    def condition(self, phase: float, y_desired_mean: npt.ArrayLike,
                  y_desired_covariance: Optional[npt.ArrayLike] = None,
                  method: str = 'conditional') -> Gaussian:
        r"""
        Modulate the trajectory distribution by conditioning.

        Compute the posterior distribution over the weights :math:`p(w | y^*)` then use this posterior to either
        compute the marginal (:math:`p(y_t; \theta)`) or conditional prediction (:math:`p(y_t|w)`).

        Args:
            phase (float): phase value.
            y_desired_mean (np.float[2D]): desired output state.
            y_desired_covariance (np.float[2D,2D], None): desired output covariance. If it's low (thus the precision
              is high), it means that we want to reach that point.
            method (str): choice between 'conditional' and 'marginal' prediction.

        Returns:
            Gaussian: the posterior distribution over the weights.
        """
        weights = self.posterior_weights(y_desired_mean, phase, y_cov=y_desired_covariance)
        return self.predict(phase, weights=weights, method=method)

    # alias
    # modulate = condition

    def combine(self, mps: List["ProMP"], priorities: List[float]) -> "ProMP":
        r"""
    ...
    def blend(self, mps: List["ProMP"], priorities: npt.ArrayLike) -> "ProMP":
        r"""
    ...
    def power(self, priority: Union[float, int, Callable]) -> "ProMP":
        """
    ...
    def multiply(self, other: "ProMP") -> "ProMP":
        r"""
        ...
        if isinstance(other, ProMP):
            ...
            raise TypeError(f"Trying to multiply a ProMP with {type(other)}, which has not be defined.")
    ...
    def __call__(self, phase: Union[float, npt.ArrayLike], probabilistic: bool = True, method: str = 'marginal',
                 return_gaussian: bool = True, sample: bool = False
                 ) -> Union[np.ndarray, Gaussian, List[Gaussian], Tuple[np.ndarray, np.ndarray]]:
        """Predict output given the phase."""
        if probabilistic:
            return self.predict_proba(phase, method=method, return_gaussian=return_gaussian)
        return self.predict(phase, method=method, sample=sample)
    ...
    def __getitem__(self, idx: Union[int, slice]) -> "ProMP":
        """
    ...
    def __pow__(self, priority: Union[float, int, Callable]) -> "ProMP":
        """Set the priority."""
        return self.power(priority=priority)

    def __mul__(self, other: "ProMP") -> "ProMP":
        """Multiply this ProMP with another one."""
        return self.multiply(other)

    def __rmul__(self, other: "ProMP") -> "ProMP":
        """Multiply this ProMP with another one."""
        return self.multiply(other)
...
class DiscreteProMP(ProMP):
    ...
    """
    def __init__(self, num_dofs: int, num_basis: int, weights: Optional[Union[npt.ArrayLike, Gaussian]]=None,
                 canonical_system: Optional[CS]=None, noise_covariance: Union[float, npt.ArrayLike]=1.,
                 basis_width: Optional[float]=None, compute_derivatives: Union[bool, List[bool]]=True) -> None:
        """
        ...
        Args:
            ...
            weights (np.float[DM], Gaussian, None): the weights that can be optimized. If None, it will create a
              custom weight array.
            canonical_system (CS, None): canonical system. If None, it will create a Linear canonical system that goes
              from `t0=0` to `tf=1`.
            noise_covariance (float, np.float[2D], np.float[2D,2D]): covariance noise matrix.
            ...
            compute_derivatives (bool, List[bool]): if we should compute the derivative of the basis functions wrt time
              as well.
        """
        if isinstance(compute_derivatives, bool):
            output_dim = 2 * num_dofs
        else:
            output_dim = np.sum([2**value for value in compute_derivatives])

        super(DiscreteProMP, self).__init__(num_dofs=num_dofs, weight_size=num_dofs*num_basis, weights=weights,
                                            output_dim=output_dim, canonical_system=canonical_system,
        ...
        if num_dofs == 1:
            self.Phi = GaussianBM(self.cs, num_basis, basis_width=basis_width, compute_derivative=compute_derivatives)
        else:
            if isinstance(compute_derivatives, bool):
                compute_derivatives = [compute_derivatives] * num_dofs
            self.Phi = BlockDiagonalMatrix([GaussianBM(self.cs, num_basis, basis_width=basis_width,
                                                       compute_derivative=compute_derivative)
                                            for compute_derivative in compute_derivatives])
...
class RhythmicProMP(ProMP):
    ...
    def __init__(self, num_dofs: int, num_basis: int, weights: Optional[Union[npt.ArrayLike, Gaussian]]=None,
        ...
        if num_dofs == 1:
            self.Phi = VonMisesBM(self.cs, num_basis, basis_width=basis_width, compute_derivative=compute_derivatives)
        else:
            if isinstance(compute_derivatives, bool):
                compute_derivatives = [compute_derivatives] * num_dofs
...
def plot_weighted_basis(t, promp, dims=[0,1], ax=None):
    """Plot weighted basis."""
    phi_track = promp.weighted_basis(t)  # shape: DM,T,2D

    if ax is None:
        fig, ax = plt.subplots(1, len(dims))

    for dim in dims:
        ax[dim].plot(phi_track[:, :, dim].T, linewidth=0.5)
