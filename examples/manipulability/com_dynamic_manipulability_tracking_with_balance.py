...
See Also:
    - `com_manipulability_tracking.py`: in this example, we track the velocity manipulability ellipsoid while the base
        is fixed.
...
if robot.name == 'nao':
    ...
    # desired dynamic Manipulability for CoM (Nao)
    des_manips = np.array([[[1.539e-03, 6.653e-04, 0.833e-04],
                           ...
                           [[1e-04, 0.0, 0.0],
                            [0.0, 5e-04, 0.0],
                            [0.0, 0.0, 1e-04]]])
    des_dyn_manip = des_manips[num_des_manip-1, :, :]
    ...
    Km = 700 * np.eye(6)  # Proportional gain for Centauro

    # desired dynamic Manipulability for CoM (Centauro)
    des_manips = np.array([[[0.321, 0.025, -0.118],
                            [0.025, 0.176, -0.058],
                            [-0.118, -0.058, 0.187]],
                           [[0.044, 0.016, 0.005],
                            [0.016, 0.464, -0.013],
                           ...
                           [[0.827, 0.096, 0.311],
                            ...
                            [0.311, -1.770, 8.547]]])
    des_dyn_manip = des_manips[num_des_manip-1, :, :]
    print("desVelMan: {}".format(des_dyn_manip))
    ...
    # Gain matrices
    Kcom = np.diag((250., 250., 0.))  # Proportional gain for CoM position control
    ...
    if init_qs == 1:
        ...
        q0 = [0.0, -1.3, 1.4, -1.2, 0.0, 1.3, -1.4, 1.2, -0.42, -0.96, -0.59,
              0.42, 0.96, 0.59, 0.42, 0.96, 0.59, -0.42, -0.96, -0.59]
        q0id = ['j_arm1_1', 'j_arm1_2', 'j_arm1_3', 'j_arm1_4', 'j_arm2_1', 'j_arm2_2', 'j_arm2_3', 'j_arm2_4',
                'hip_pitch_1', 'knee_pitch_1', 'ankle_pitch_1', 'hip_pitch_2', 'knee_pitch_2', 'ankle_pitch_2',
                'hip_pitch_3', 'knee_pitch_3', 'ankle_pitch_3', 'hip_pitch_4', 'knee_pitch_4', 'ankle_pitch_4']
...
H0 = robot.get_mass_matrix(q0)
if not robot.has_fixed_base():
    dynMan = robot.compute_dynamic_manipulability_ellipsoid(Jcom0[:, 6:], H0[6:, 6:])
else:
    dynMan = robot.compute_dynamic_manipulability_ellipsoid(Jcom0, H0)
print("Mv0: {}".format(dynMan[0:3, 0:3]))

# Logging variables
# Format: [q minEigvalue(Jbal) minEigvalue(Jman) balanceError CurrentManip(1x9) SPDdistance]
num_samples = 400
log_array = np.zeros((num_samples, num_dofs + 2 + Kbal.shape[0] + dynMan[0:3, 0:3].size + 1))
...
for i in range(num_samples):
    ...
    # Tracking of CoM dynamic manipulability in nullspace
    Ht = robot.get_mass_matrix(qt)
    if not robot.has_fixed_base():
        dynMan = robot.compute_dynamic_manipulability_ellipsoid(Jcom[:, 6:], Ht[6:, 6:])
    else:
        dynMan = robot.compute_dynamic_manipulability_ellipsoid(Jcom, Ht)

    # Obtaining joint velocity command
    if not robot.has_fixed_base():
        dq_man, minSman, SPDdist = robot.calculate_inverse_differential_kinematics_dynamic_manipulability(Jcom[:, 6:],
                                                                                                          Ht[6:, 6:],
                                                                                                          des_dyn_manip,
                                                                                                          Km)
    else:
      dq_man, minSman, SPDdist = robot.calculate_inverse_differential_kinematics_dynamic_manipulability(Jcom, Ht,
                                                                                                        des_dyn_manip,
                                                                                                        Km)
    ...
    log_array[i, ] = np.hstack((qt.reshape(1, num_dofs), np.min(Sbal).reshape(1, 1),
                               minSman.reshape(1, 1), dx_bal.T, dynMan[0:3, 0:3].reshape(1, dynMan[0:3, 0:3].size),
                               SPDdist.reshape(1, 1)))
